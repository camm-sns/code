...
python sassenatasks.py genSQE fqt.hd5 simulated.nxs --wsname simulated --indexes "2 4 6 8" --LoadSassena "TimeUnit:1.0" --SassenaFFT "FFTonlyRealPart:1,DetailedBalance:1,Temp:290" --NormaliseToUnity "RangeLower:-0.1,RangeUpper:0.1"
  ...
  Inputs:
	...
	fqt.hd5 a sassena file containing ten histograms for I(Q,t). Each histogram has a different Q-value,
	with Q from 0.1 to 1.0 every 0.1Angstroms^(-1).  Of these, we only transform to S(Q,E) those
	histograms with Q-values 0.3, 0.5, 0.7, and 0.9, corresponding to workspace indexes 2, 4, 6, and
	8 (--indexes "2 4 6 8")
